...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //the 'data' property. The value is 'songData'. That means ng-grid is looking for songData on $scope and is putting 
  //whatever songData is into the grid.
  //this means when you make your iTunes request, you'll need to get back the information, parse it accordingly, then set 
  //it to songData on the scope -> $scope.songData = ...
  $scope.gridOptions = { 
      data: 'songData',
      height: '110px',
  ...
  //Our controller is what's going to connect our 'heavy lifting' itunesService with our view (index.html) so our user can 
  //see the results they get back from itunes.
  ...
  //that's not what ng-grid is expecting. What you need to do now
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through 
  //the iTunes data pushing into your new array objects that look like the above data.
    ...
    var sortResults = function () {
      for (i = 0; i < rawData.length; i++) {
        $scope.songData.push({
            ...
            Collection: rawData[i].collectionCensoredName,
  ...
  //Once you have that final data array, you simply need to put it on the scope (or more specifically on the scope as 
  //songData). Once you do this ($scope.songData = myFinalArray) then ng-grid will see that and populate the page.
